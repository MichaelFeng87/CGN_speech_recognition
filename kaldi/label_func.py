...
def parse_transcript (pattern, loc):
    # some minor differences with parse_transcript in the preprocessing folder: there
    # is no CGN option as we always use CGN and since we do not have to replace "" with 'sil'
    # as we add sil to the lexicon by hand
    
    # parse the raw transcript
    regex= re.compile(pattern)
    # some CGN transcripts are zipped
    try:
    ...
    # 4th line of each transcript should say how many entries there are, multiply by three because each entry has 3 lines.
